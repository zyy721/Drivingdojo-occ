...
from diffusers.pipelines.stable_video_diffusion.pipeline_stable_video_diffusion import StableVideoDiffusionPipeline, _append_dims, retrieve_timesteps, _resize_with_antialiasing
...
class StableVideoDiffusionPipelineMultiviewCondDome(StableVideoDiffusionPipeline):
    r"""
    ...
    def _encode_image(
        ...
        image_list = []
        for cur_cam_img in input_dict['images'][0]:
    ...
    def __call__(
        ...
        image_embeddings = self._encode_image(input_dict, device, num_videos_per_prompt, self.do_classifier_free_guidance)
        ...
        # (1, 64, 2, 25, 25)
        occ_latent = torch.cat([occ_latent] * 2) if self.do_classifier_free_guidance else occ_latent
        # (6, 2, 3, 3)
        metas[0]['new_cam_intrinsic'] = np.concatenate([metas[0]['new_cam_intrinsic']] * 2) if self.do_classifier_free_guidance else metas[0]['new_cam_intrinsic']
        metas[0]['new_cam2ego'] = np.concatenate([metas[0]['new_cam2ego']] * 2) if self.do_classifier_free_guidance else metas[0]['new_cam2ego']
