...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=int,
        default=10,
        help="Number of past video frames.",
    )
...
def main():
    ...
    nframes_past = args.nframes_past
    ...
    unet = UNetSpatioTemporalConditionModelMultiviewCondDome.from_unet_spatio_temporal_condition(unet_origin, **unet_param)
    ...
    # load config
    cfg = Config.fromfile('examples/world_model/utils/DOME/config/custom_train_dome.py')
    ...
    with temporarily_disable_deepspeed_zero3():
        ...
        occ_vae=MODELS.build(cfg.model.vae)
        # load vae
        # assert cfg.vae_load_from and osp.exists(cfg.vae_load_from)
        ckpt_occ_vae = torch.load(cfg.vae_load_from, map_location='cpu')
        if 'state_dict' in ckpt_occ_vae:
            state_dict = ckpt_occ_vae['state_dict']
        else:
            state_dict = ckpt_occ_vae
        print(occ_vae.load_state_dict(state_dict, strict=False))
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet):
                ...
                input_occs, target_occs, pixel_values, images, metas = batch

                x=input_occs
                bs, _, _, _, _ = x.shape
                x, shape = occ_vae.forward_encoder(x) #16 128 50 50
                # vae sample
                x,_,_=occ_vae.sample_z(x) #16 64 50 50
                ...
                model_pred = unet(input_latents, c_noise, encoder_hidden_states=encoder_hidden_states, added_time_ids=add_time_ids, occ_latent=occ_latent, metas=metas).sample
