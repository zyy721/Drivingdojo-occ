...
from src.pipeline.pipeline_stable_video_diffusion_multiview_clean_latent_singleview import StableVideoDiffusionPipelineMultiviewCleanLatentSingleview
...
unet = UNetSpatioTemporalConditionModelSingleview.from_pretrained(unet_path, torch_dtype=torch.float16)
unet.eval()
...
#     pretrained_model_path, **pipe_param, torch_dtype=torch.float16, variant="fp16"
# )
pipe = StableVideoDiffusionPipelineMultiviewCleanLatentSingleview.from_pretrained(
    pretrained_model_path, **pipe_param, torch_dtype=torch.float16, variant="fp16"
)
...
with torch.no_grad():
    for batch_dict in val_dataloader:
        ...
        for idx_frame in range(length):
            ...
            cat_cur_multiview_imgs = frames[0][idx_frame]
