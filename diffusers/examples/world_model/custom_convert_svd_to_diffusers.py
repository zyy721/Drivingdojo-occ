...
def convert_ldm_unet_checkpoint(checkpoint, config, path=None, extract_ema=False, skip_extract_state_dict=False):
    ...
    #     if "cond_time_stack_embed" in k:  # duplicate a new timestep embedding from the pretrained weights
    #         time_embed_key_list.append(k)

    new_checkpoint = {}

    new_checkpoint["cond_time_stack_embedding.linear_1.weight"] = unet_state_dict["cond_time_stack_embed.0.weight"]
    new_checkpoint["cond_time_stack_embedding.linear_1.bias"] = unet_state_dict["cond_time_stack_embed.0.bias"]
    new_checkpoint["cond_time_stack_embedding.linear_2.weight"] = unet_state_dict["cond_time_stack_embed.2.weight"]
    new_checkpoint["cond_time_stack_embedding.linear_2.bias"] = unet_state_dict["cond_time_stack_embed.2.bias"]
...
def main():
    cfg = "./utils/Vista/configs/training/vista_phase2_stage2.yaml"
    config = OmegaConf.load(cfg)
    vista_config = create_unet_diffusers_config(config, 768)

    vista_svd = load_safetensors("./demo_model/vista.safetensors")
    vista_state_dict = convert_ldm_unet_checkpoint(vista_svd, vista_config)

    save_file(vista_state_dict, "./demo_model/diffusion_vista.safetensors")

    # unet = UNetSpatioTemporalConditionModel.from_pretrained('demo_model/img2video_1024_14f', subfolder="unet")
